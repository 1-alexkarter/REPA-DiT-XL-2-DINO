...
class Interfaces(nnx.Module, ABC):
    ...
    def __init__(
        ...
        if isinstance(train_time_dist_type, str):
            self.train_time_dist_type = TrainingTimeDistType[
                train_time_dist_type.replace("_", "").upper()
            ]
        else:
            self.train_time_dist_type = train_time_dist_type

        # Min-SNR-γ clipping threshold (None = disabled)
    ...
    # Min-SNR-γ reweighting helper
    def _min_snr_weight(self, snr: jnp.ndarray) -> jnp.ndarray:
        """Compute w(t) = min(SNR(t), γ) / SNR(t) (γ from self.min_snr_gamma).
        If min_snr_gamma is None, returns 1.
        """
        if self.min_snr_gamma is None:
            return jnp.ones_like(snr)
        gamma = jnp.asarray(self.min_snr_gamma, dtype=snr.dtype)
        return jnp.minimum(snr, gamma) / (snr + 1e-8)
...
class SiTInterface(Interfaces):
    ...
    def __init__(
        ...
        super().__init__(network, train_time_dist_type, min_snr_gamma=min_snr_gamma)
        self.t_mu = t_mu
    ...
    def sample_t(self, shape: tuple[int, ...]) -> jnp.ndarray:
        ...
        if self.train_time_dist_type == TrainingTimeDistType.UNIFORM:
            ...
            return jax.nn.sigmoid(
                jax.random.normal(rng, shape=shape) * self.t_sigma + self.t_mu
            )
        else:
            raise ValueError(
                f"Training Time Distribution Type {self.train_time_dist_type} not supported."
            )
    ...
    def loss(self, x: jnp.ndarray, *args, return_aux=False, **kwargs) -> jnp.ndarray:
        ...
        if return_aux:
            ...
            return {"loss": self.mean_flat((net_out - target) ** 2)}
...
class EDMInterface(Interfaces):
    ...
    def pred(self, x_t: jnp.ndarray, t: jnp.ndarray, *args, **kwargs) -> jnp.ndarray:
        ...
        D_x = (
            self.bcast_right(self.c_skip(t), x_t) * x_t
            + self.bcast_right(self.c_out(t), F_x) * F_x
        )
    ...
    def loss(self, x: jnp.ndarray, *args, return_aux=False, **kwargs) -> jnp.ndarray:
        ...
        F_x, features = self.network(
            (self.bcast_right(self.c_in(sigma), x_t) * x_t),
            self.c_noise(sigma),
            *args,
            **kwargs,
        )
        D_x = (
            self.bcast_right(self.c_skip(sigma), x_t) * x_t
            + self.bcast_right(self.c_out(sigma), F_x) * F_x
        )
        gamma = getattr(self, "min_snr_gamma", None)  # or read from config

        snr = (self.x_sigma / sigma) ** 2  # SNR = sigma_data^2 / sigma^2

        if gamma is not None:
            # Min-SNR-γ clipping
            snr_clipped = jnp.minimum(snr, gamma)
            # Loss weight up to a constant scale; EDM-style is ~ (1 + SNR)
            weight = 1.0 + snr_clipped
        else:
            # No clipping
        ...
        if return_aux:
            # specifically for auxiliary loss wrappers
            return (
                self.mean_flat(self.bcast_right(weight, D_x) * (D_x - target) ** 2),
                D_x,
                features,
            )
        else:
            return {
                "loss": self.mean_flat(
                    self.bcast_right(weight, D_x) * (D_x - target) ** 2
                )
            }
...
class MeanFlowInterface(SiTInterface):
    ...
    def __init__(
        ...
        x_sigma: float = 0.5,
        guidance_scale: float = 1.0,
        guidance_mixture_ratio: float = 0.5,
        guidance_t_min: float = 0.0,
        guidance_t_max: float = 1.0,
        norm_eps: float = 1e-3,
        norm_power: float = 1.0,
        fm_portion: float = 0.75,
        cond_drop_ratio: float = 0.5,
        t_shift_base: int = 4096,
    ):
        super().__init__(
            network,
    ...
    def cond_drop(
        ...
        mask = (
            jax.random.uniform(self.network.rngs.label_dropout(), shape=y.shape)
            < self.cond_drop_ratio
        )
        ...
        **kwargs,
        ...
        if self.guidance_mixture_ratio == 0.0:
            return (
                jnp.where(
                    self.bcast_right(
                        (t >= self.guidance_t_min) & (t <= self.guidance_t_max), v
                    ),
        ...
        return (
            jnp.where(
                ...
                + (1 - self.guidance_scale - self.guidance_mixture_ratio) * v_uncond
                + self.guidance_mixture_ratio * v_cond,
                v,
            ),
            y,
            neg_y,
    ...
    def target(
        ...
        **kwargs,
        ...
        u, dudt, feat = jax.jvp(u_fn, (x_t, t, r), (v, dtdt, drdt), has_aux=True)

        return (u, feat), jax.lax.stop_gradient(
            v - jnp.clip(self.bcast_right(t - r, v), 0.0, 1.0) * dudt
        )

    def pred(
        self, x_t: jnp.ndarray, t: jnp.ndarray, r: jnp.ndarray, *args, **kwargs
    ) -> jnp.ndarray:
        ...
        """
        return self.network(
            (self.bcast_right(self.c_in(t), x_t) * x_t), t, *args, dt=t - r, **kwargs
        )[0]
