...
def get_config(options="imagenet_64-B_2"):
    ...
    data_preset = ml_collections.ConfigDict(
        common_specs._imagenet_data_presets[data_options]
    )
    encoder_preset = ml_collections.ConfigDict(
        common_specs._imagenet_encoder_presets[data_options]
    )

    network_options, patch_size = network_options.split("_")
    network_preset = ml_collections.ConfigDict(
        common_specs._dit_network_presets[network_options]
    )
    ...
    # inputs
    config.network.input_size = int(data_preset.image_size) // config.encoder.get(
        "downsample_factor", 1
    )
    ...
    config.interface.min_snr_gamma = 5.0

    # REPA: took from https://github.com/sihyun-yu/REPA/blob/main/train.py
    config.repa = ml_collections.ConfigDict()
    config.repa_class = "repa"
    config.repa.loss = ml_collections.ConfigDict()
    config.repa.loss.repa_loss_weight = 0.5
    config.repa.loss.repa_depth = 4
    config.repa.loss.proj_dim = 2048

    config.repa.detector = ml_collections.ConfigDict()
    config.repa.detector_class = "dino_reg"
    config.repa.detector.pretrained_path = "facebook/dinov2-with-registers-base"
    # config.repa.detector_class = "dino"
    # config.repa.detector.pretrained_path = "facebook/dinov2-base"
    ...
    config.sampler_class = "euler-maruyama"
    config.sampler.num_sampling_steps = 250
    config.sampler.sampling_time_dist = "uniform"
    config.sampler.sampling_time_kwargs = ml_collections.ConfigDict()
    config.sampler.sampling_time_kwargs.t_start = 0.999
    config.sampler.sampling_time_kwargs.t_end = 0.0
