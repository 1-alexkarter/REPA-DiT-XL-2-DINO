...
from networks.encoders import dino, rae, dino_w_register
...
DETECTOR_REGISTRY = {
    "dino": dino.DINO,
    "dino_reg": dino_w_register.DinoWithRegisters,  # DINOv2-with-registers
}
...
def create_learning_rate_fn(
    ...
    warmup_fn = optax.linear_schedule(
        init_value=0.0, end_value=learning_rate, transition_steps=config.warmup_steps
    )
    ...
    linear_fn = optax.linear_schedule(
        init_value=learning_rate, end_value=0.0, transition_steps=cosine_epochs
    )
    ...
    polynomial_fn = optax.polynomial_schedule(
        init_value=learning_rate, end_value=0.0, power=2, transition_steps=cosine_epochs
    )
    ...
    if config.learning_rate_schedule == "cosine":
        ...
        schedule_fn = optax.join_schedules(
            schedules=[warmup_fn, linear_fn], boundaries=[config.warmup_steps]
        )
    elif config.learning_rate_schedule == "constant":
        schedule_fn = constant_fn
    elif config.learning_rate_schedule == "polynomial":
        schedule_fn = optax.join_schedules(
            schedules=[poly_warmup_fn, polynomial_fn], boundaries=[config.warmup_steps]
        )
    elif config.learning_rate_schedule == "linear-constant":
        schedule_fn = optax.join_schedules(
            schedules=[warmup_fn, constant_fn], boundaries=[config.warmup_steps]
        )
        ...
        def schedule_fn(step):
            ...
            if config.lr_rampup_Mimg > 0:
                lr *= jax.numpy.minimum(
                    step / (config.lr_rampup_Mimg * 1e6) * batch_size, 1
                )
...
def instantiate_network(config: ml_collections.ConfigDict):
    ...
    base_rngs = nnx.Rngs(
        ...
        time=seed + 2,
        noise=seed + 3,
        label_dropout=seed + 4,
    )
    network = MODEL_REGISTRY[network_class](
        **config.network, dtype=dtype, rngs=base_rngs
    )
    return network
...
def instantiate_optimizer(
    ...
    base_opt = OPTIMIZER_REGISTRY[config.optimizer_class](
        learning_rate=learning_rate_fn, **config.optimizer
    )
    # optional global-norm clipping
    max_norm = getattr(config, "max_grad_norm", 0.0)
...
def instantiate_ema(config: ml_collections.ConfigDict, model: nnx.Module):
