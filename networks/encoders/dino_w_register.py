...
class FlaxDinov2WithRegistersEmbeddings(nn.Module):
    ...
    def interpolate_pos_encoding(
        ...
        patch_pos_embed_expanded = jnp.tile(
            patch_pos_embed, (hidden_states.shape[0], 1, 1)
        )
        class_pos_embed_expanded = jnp.tile(
            class_pos_embed, (hidden_states.shape[0], 1, 1)
        )

        return jnp.concatenate(
            (class_pos_embed_expanded, patch_pos_embed_expanded), axis=1
        )
...
class FlaxDinov2WithRegistersAttention(nn.Module):
    ...
    def setup(self):
        self.attention = FlaxDinov2WithRegistersSelfAttention(
            self.config, dtype=self.dtype
        )
...
class FlaxDinov2WithRegistersLayer(nn.Module):
    ...
    def setup(self):
        ...
        self.layer_scale1 = FlaxDinov2WithRegistersLayerScale(
            self.config, dtype=self.dtype
        )
        ...
        self.layer_scale2 = FlaxDinov2WithRegistersLayerScale(
            self.config, dtype=self.dtype
        )
...
class FlaxDinov2WithRegistersEncoder(nn.Module):
    ...
    def setup(self):
        self.layer = FlaxDinov2WithRegistersLayerCollection(
            self.config, dtype=self.dtype
        )
...
class FlaxDinov2WithRegistersModule(nn.Module):
    ...
    def setup(self):
        self.embeddings = FlaxDinov2WithRegistersEmbeddings(
            self.config, dtype=self.dtype
        )
...
class FlaxDinov2WithRegistersForImageClassificationModule(nn.Module):
    ...
    def setup(self):
        self.dinov2 = FlaxDinov2WithRegistersModule(
...
class FlaxDinov2WithRegistersForImageClassification(
    FlaxDinov2WithRegistersPreTrainedModel
):
...
overwrite_call_docstring(
    FlaxDinov2WithRegistersForImageClassification, FLAX_VISION_CLASSIFICATION_DOCSTRING
)
append_replace_return_docstrings(
    FlaxDinov2WithRegistersForImageClassification,
            ...
            if "weight" in th_name:
                if "norm" in th_name:
                    ...
                    if param.ndim == 2:
                        ...
                        flat_params[name] = (
                            th_params[th_name].cpu().numpy().transpose(2, 3, 1, 0)
                        )
                    else:
...
class DinoWithRegisters(nnx.Module):
    ...
    def __init__(
        ...
        network_params = network.init(
            ...
            jnp.zeros(
                (1, self.config.image_size, self.config.image_size, 3), dtype=dtype
            ),
        )
        network_params = unfreeze(network_params)
    ...
    def encode(self, x: jnp.ndarray, deterministic: bool = True) -> jnp.ndarray:
        # x = self.preprocess(x)
        return self.network(
            x, output_hidden_states=True, deterministic=deterministic
        ).last_hidden_state[:, 5:]
