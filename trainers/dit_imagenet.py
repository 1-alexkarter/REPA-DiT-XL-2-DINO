...
def train_step(
    ...
    #     initializer=0.0
    # )
    grad_sq = jax.tree_util.tree_reduce(
        lambda a, b: a + b,
        jax.tree_util.tree_map(lambda g: jnp.sum(jnp.square(g)), grads),
        initializer=0.0,
    )
    grad_norm = jnp.sqrt(grad_sq + 1e-9)
    ...
    if hasattr(model, "interface"):
        ...
        ema.update(model)
    metric_dict = {loss_type: loss.mean() for loss_type, loss in loss_dict.items()}
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    wandb_utils.initialize(config, exp_name=exp_name, project_name=project_name)
    ...
    encoder, model, optimizer, sampler, ema, learning_rate_fn = init_utils.build_models(
        config
    )
    ...
    if config.standalone_eval:
        restore_step = (
            config.get("restore_step")
            if config.get("restore_step")
    ...
    mesh = sharding_utils.create_device_mesh(
        ...
        allow_split_physical_axes=config.sharding.get(
    ...
    if config.visualize.get("on"):
        vis_utils.visualize(config, model, ema.ema, encoder, sampler, step, mesh=mesh)
        if config.visualize.get("visualize_reconstruction"):
            ...
            vis_utils.visualize_reconstruction(config, encoder, in_x, mesh=mesh)
    ...
    if config.eval.get("on") and config.eval.get("on_load"):
        for guidance_scale, sample_sizes in zip(
            config.eval.all_guidance_scales, config.eval.all_eval_samples_nums
        ):
            fid.calculate_fid(
                config,
                dataset,
        ...
        with jax.profiler.StepTraceAnnotation("train", step_num=step):
            with mesh:
                state, ema_state, metric_dict = p_train_step(
                    ...
                    graphdef,
                    ema_graphdef,
                    config.optimizer.get("max_grad_norm", None),
                )
